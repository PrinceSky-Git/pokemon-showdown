...
interface UserSettings {
  [userid: string]: {
    ...
    showcaseSort?: 'rarity' | 'points' | 'type' | 'name' | 'date';
  };
}
...
function genCard(setCode?: string): Card | null {
  const validCards = Object.values(cardDefinitions).filter(card => {
    if (setCode && card.setCode !== setCode) return false;
    return true;
  });
...
function makePack(setCode: string): CardInstance[] {
  const out: CardInstance[] = [];
  const packCards = Object.values(cardDefinitions).filter(c => c.setCode === setCode);
  
  if (!packCards.length) return out;

  // Generate cards based on rarity distribution
  for (let i = 0; i < CARDS_PER_PACK; i++) {
    const card = genCard(setCode);
    if (card) {
...
function displayCard(card: Card): string {
  return `<div style="display: flex; gap: 20px; flex-wrap: wrap;">` +
    ...
    `<div style="font-size: 1.5em; font-weight: bold; color: #cc0000; margin-bottom: 15px;">${card.rarity}</div>` +
    `<div style="margin-bottom: 8px;"><strong>Type:</strong> ${card.type}${card.secondaryType ? ` / ${card.secondaryType}` : ''}</div>` +
    `<div style="margin-bottom: 8px;"><strong>Super Type:</strong> ${card.superType}</div>` +
    `<div style="margin-bottom: 8px;"><strong>Sub Type:</strong> ${card.subType}</div>` +
    `<div style="margin-bottom: 8px;"><strong>Set:</strong> ${card.setCode}</div>` +
    `<div style="margin-bottom: 8px;"><strong>Number:</strong> ${card.number}</div>` +
...
export const commands: Chat.Commands = {
  psgo: {
    card: {
      ...
      async transfer(target, room, user, connection, cmd) {
        ...
        const card = cardDefinitions[toID(cardId)];
        if (!card) return this.errorReply('That card does not exist.');
        ...
        if (cmd !== 'confirmtransfer') {
          return this.popupReply(
            `|html|<center>` +
            `<button class="button" name="send" value="/psgo card confirmtransfer ${targetUser.id}, ${card.id}" ` +
      ...
      transferhelp: ['/psgo card transfer [user], [card id] - Transfer a specified card to another user.'],
      ...
      async transferall(target, room, user, connection, cmd) {
        ...
        if (cmd !== 'confirmtransferall') {
          return this.popupReply(
            `|html|<center>` +
            `<p>Are you sure you want to transfer ALL ${userCards.length} cards to ${targetUser.name}?</p>` +
            `<button class="button" name="send" value="/psgo card confirmtransferall ${targetUser.id}" ` +
            `style="padding: 15px 30px; font-size: 14px; border-radius: 8px;">` +
      ...
      add(target, room, user) {
        ...
        const [id, name, image, subType, superType, number, rarity, pointsStr, setCode, type, secondaryType] = 
          target.split(',').map(x => x.trim());
        
        if (!type) return this.parse('/help psgo card add');
        
        const cardId = toID(id);
        ...
        cardDefinitions[cardId] = {
          ...
          setCode,
          type,
          secondaryType: secondaryType === 'NULL' ? null : secondaryType,
        };
        
        saveCardDefinitions();
        
        this.modlog('PSGO CARD ADD', null, cardId);
        return this.sendReply(`Card ${cardId} has been added.`);
      },
      addhelp: ['/psgo card add [id], [name], [image], [subType], [superType], [number], [rarity], [points], [setCode], [type], [secondaryType] - Adds a card. Requires: Manager or #'],
      ...
      edit(target, room, user) {
        ...
        if (!type) return this.parse('/help psgo card edit');
        ...
        return this.sendReply(`Card ${cardId} has been updated.`);
      },
      edithelp: ['/psgo card edit [id], [name], [image], [subType], [superType], [number], [rarity], [points], [setCode], [type], [secondaryType] - Edits a card. Requires: Manager or #'],
      ...
      delete(target, room, user) {
        ...
        delete cardDefinitions[cardId];
        saveCardDefinitions();
        
        this.modlog('PSGO CARD DELETE', null, cardId);
        return this.sendReply(`${cardId} has been removed from the card database.`);
      },
      deletehelp: ['/psgo card delete [id] - Deletes a card. Requires: Manager or #'],
      ...
      givehelp: ['/psgo card give [user], [card id] - Gives the user the specified card. Requires: Manager or @ ~'],
      ...
      takehelp: ['/psgo card take [user], [card id] - Takes the specified card from the user. Requires: Manager or @ ~'],
    },
    ...
    pack: {
      shop(target, room, user) {
        ...
        const packsHTML = shopPacks.map(pack => 
          `<tr>` +
          `<td style="padding: 10px;"><button class="button" name="send" value="/psgo pack shop buy ${pack.code}">${pack.name}</button></td>` +
      ...
      shophelp: ['/psgo pack shop - Shows the shop for buying packs.'],

      async buy(target, room, user) {
        if (!target) return this.parse('/help psgo pack shop buy');
      ...
      buyhelp: ['/psgo pack shop buy [pack] - Buy a pack from the shop.'],
      ...
      async open(target, room, user) {
        ...
        return this.sendReplyBox(
          `<div style="margin-bottom: 10px;">You opened a <b>${packName}</b> pack and received:</div>` +
          `<div>${cardsHTML}</div>`
        );
      },
      ...
      async transfer(target, room, user, connection, cmd) {
        ...
        if (cmd !== 'confirmtransfer') {
          return this.popupReply(
            `|html|<center>` +
            `<button class="button" name="send" value="/psgo pack confirmtransfer ${targetUser.id}, ${code}" ` +
            `style="padding: 15px 30px; font-size: 14px; border-radius: 8px;">` +
    ...
    async showcase(target, room, user) {
      ...
      // Get user's showcase sort preference
      const settings = await loadUserSettings();
      ...
      switch (sortType) {
        case 'points':
          sortedCards.sort((a, b) => (b.points || 0) - (a.points || 0));
          break;
        case 'type':
    ...
    async sortshowcase(target, room, user) {
      ...
      const validTypes = ['rarity', 'points', 'type', 'name', 'date'];
    ...
    sortshowcasehelp: ['/psgo sortshowcase [type] - Changes the order of how cards are displayed in your showcase. Types: rarity, points, type, name, date'],
  ...
  psgohelp: [
    'User Commands:',
    '/psgo card show [id] - Displays the details of a card.',
    '/psgo card transfer [user], [card id] - Transfer a specified card to another user.',
    ...
    '/psgo card add [id], [name], [image], [subType], [superType], [number], [rarity], [points], [setCode], [type], [secondaryType] - Adds a card. Requires: Manager or #',
    '/psgo card edit [id], [name], [image], [subType], [superType], [number], [rarity], [points], [setCode], [type], [secondaryType] - Edits a card. Requires: Manager or #',
    '/psgo card delete [id] - Deletes a card. Requires: Manager or #',
    ...
    '/psgo card give [user], [card id] - Gives the user the specified card. Requires: Manager or @ ~',
    '/psgo card take [user], [card id] - Takes the specified card from the user. Requires: Manager or @ ~',
    ...
    if (action === 'collection') {
      ...
      for (const rarity of rarityOrder) {
        ...
        for (const card of cardsByRarity[rarity]) {
          output += `<button class="button" name="send" value="/psgo card show ${card.id}" style="padding: 0;">` +

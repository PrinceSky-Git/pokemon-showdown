    ...
    async psgo(args, user) {
        ...
        if (action === 'collection') {
            ...
            for (const card of cards) {
                const normalizedRarity = normalizeRarity(card.rarity);
                if (!cardsByRarity[normalizedRarity]) cardsByRarity[normalizedRarity] = [];
                cardsByRarity[normalizedRarity].push(card);
            }
            ...
            for (const rarity of rarityOrder) {
                ...
                const rarityColor = RARITY_COLORS[rarity] || '#cc0000';
                output += `<h3 style="color: ${rarityColor}">${rarity} (${cardsByRarity[rarity].length})</h3>`;

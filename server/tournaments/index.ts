...
export class Tournament extends Rooms.RoomGame<TournamentPlayer> {
	...
	*/
	...
	private async distributeTournamentRewards() {
		...
		// Only give rewards in eligible rooms
		if (!rewardEligibleRooms.includes(this.room.roomid)) return;
		...
		for (let place = 0; place < results.length && place < 2; place++) {
			...
			for (const player of playersAtPlace) {
				...
				try {
					...
					if (isWinner) {
						const card = Impulse.randomGiveCardWeightedSync(player.id, true);
						if (card.success) {
						this.room.add(`|html|<div class="broadcast-green">${player.name} earned <b>${rewardAmount} ${Impulse.currency}</b> and <b>${card.cardName}</b> from ${card.packName} for winning the tournament!</div>`);
						}
					} else {
						const card2 = Impulse.randomGiveCardWeightedSync(player.id, false);
						if (card2.success) {
						this.room.add(`|html|<div class="broadcast-blue">${player.name} earned <b>${rewardAmount} ${Impulse.currency}</b> and <b>${card2.cardName}</b> from ${card2.packName} for 2nd place!</div>`);
						}
				}
